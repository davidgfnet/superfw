...
# BOARD can be "sd" or "lite" or "chis"
BOARD ?= sd

ifeq ($(BOARD),lite)
  GLOBAL_DEFINES ?= -DSUPERCARD_LITE_IO -DSUPERCARD_LITE_READ -DSUPERCARD_LITE_ADDR_SORT
else ifeq ($(BOARD),chis)
  GLOBAL_DEFINES ?= -DSUPERCHIS -DSUPERCARD_LITE_READ -DSUPERCARD_LITE_ADDR_SORT -DBUNDLE_GBC_EMULATOR -DWRITE_BUF_SIZE=512 -DSECTOR_SIZE=0x20000
else ifeq ($(BOARD),sd)
